...
describe Identity do
  ...
  context "with existing account" do
    ...
    specify "facebook client is available" do
      credentials = Account.create!(:realm_id => 1, :identity => user, :provider => :facebook, 
        :token => 'facebook_token', :secret => 'facebook_secret', :uid => 'abc123')
      user.client_for(:facebook).class.should eq(FacebookClient)
    end

    specify "twitter client is available" do
      credentials = Account.create!(:realm_id => 1, :identity => user, :provider => :twitter, 
        :token => 'twitter_token', :secret => 'twitter_secret', :uid => 'abc123')
      user.client_for(:twitter).class.should eq(TwitterClient)
    end

    specify "origo client is available" do
      credentials = Account.create!(:realm_id => 1, :identity => user, :provider => :origo, 
        :token => 'origo_token', :secret => 'origo_secret', :uid => 'abc123')
