...
describe "Authorizations" do
  ...
  context "logging out" do
    ...
    xit 'logout with Vanilla account' do
      let :realm do
        Realm.create!(
          :label => "area51",
          :service_keys => YAML.dump(
            :vanilla => {
          :store => 'mystore'
        }))
      end
      ...
      # TODO: See auth.rb
      if false
        it 'calls Vanilla logout if there is an account' do
          vanilla_stub = stub_request(:post, %r{/api/vanilla/v1/mystore/logout/123}).
          ...
          vanilla_stub.should have_been_requested
        end

        it 'accepts Vanilla logout failure silently if non-5xx' do
          vanilla_stub = stub_request(:post, %r{/api/vanilla/v1/mystore/logout/123}).
